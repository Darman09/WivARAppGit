...
function modifyUser(){
	...
	if (nomUser === '') {
		console.log(ageUser);
		console.log(ageUser);
		Materialize.toast("Nom du compte manquant", 3000);
		return;
	}
	if (genreUser === '') {
		Materialize.toast("Civilité manquante", 3000);
		return;
	}
	if (ageUser === '') {
		Materialize.toast("Age manquant", 3000);
		return;
	}
	if (telephoneUserModify === '') {
		Materialize.toast("Telephone manquant", 3000);
		return;
	}
	db.collection("users").doc("${idUserConnected}").update({
	...
	<form method="POST" action="account" id="formAccount">
		<nav>
	...
	<div class="contenair">
		<div class="row">
			...
			<%@include file="/JSP/modal/modalModifyAccount.jsp"%>
		</div>
	</div>
</body>
